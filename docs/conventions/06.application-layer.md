...
/**
 * 사용자 정보를 업데이트합니다.
 * 이 메서드는 사용자 존재 여부를 별도로 체크하지 않고, 도메인 서비스에 위임합니다.
 *
 * @param criteria 사용자 업데이트 요청 기준
 * @return 업데이트된 사용자 정보
 * @throws UserException.NotFound 사용자가 존재하지 않는 경우 (도메인 계층에서 발생)
 */
@Transactional
fun updateUser(criteria: UserCriteria.Update): UserResult.Single {
    // 별도의 존재 여부 체크 없이 도메인 서비스 호출
    val updatedUser = userService.updateUser(criteria.toCommand())
    return UserResult.Single.from(updatedUser)
}
```