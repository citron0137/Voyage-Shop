# 13. 애플리케이션 레이어 동시성 테스트 컨벤션

## 1. 개요

본 문서는 Voyage-Shop 애플리케이션의 동시성 테스트 작성 규약을 정의합니다. 이 문서는 [09. 테스트 컨벤션](./09.test-conventions.md) 문서의 규약을 기반으로 합니다.
...
inner class StockExhaustion {
    ...
    fun whenLimitedStockProduct_preventsOverselling() {
        ...
        for (i in 0 until threadCount) {
            executor.submit {
                try {
                    val success = orderFacade.placeOrder(OrderCriteria.Create(
                        userId = "user$i",
    ...
    inner class ConcurrentPaymentProcessing {
        ...
        fun whenPayingConcurrently_preventsDuplicatePayments() {
            ...
            for (i in 1..threadsCount) {
                executor.submit {
                    try {
                        val result = paymentFacade.processPayment(PaymentCriteria.Create(
                            orderId = orderId,
                            amount = 10000,
                            method = "CARD"
                        ))
