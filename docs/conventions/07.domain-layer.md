...
```kotlin
// 지양해야 할 예시: 다른 도메인의 리포지토리 직접 주입
@Service
class OrderService(
    private val orderRepository: OrderRepository,
    private val productRepository: ProductRepository, // 다른 도메인의 리포지토리 직접 사용 - 지양
    private val userRepository: UserRepository // 다른 도메인의 리포지토리 직접 사용 - 지양
) {
    // ...
}

// 지양해야 할 예시: 다른 도메인의 서비스 직접 사용
@Service
class OrderService(
    private val orderRepository: OrderRepository,
    private val productService: ProductService, // 다른 도메인의 서비스 직접 사용 지양
    private val userService: UserService // 다른 도메인의 서비스 직접 사용 지양
) {
    // ...
}
```
...
```kotlin
// 권장되는 예시: 자신의 도메인 리포지토리만 사용
@Service
class OrderService(
    private val orderRepository: OrderRepository
) {
    // 자신의 도메인 데이터만 직접 처리
    fun getOrder(command: OrderCommand.GetById): Order {
        return orderRepository.findById(command.orderId)
            ?: throw OrderException.NotFound("Order not found: ${command.orderId}")
    }
}

// 애플리케이션 레이어에서 도메인 서비스 조합
@Service
class OrderApplicationService(
    ...
    fun processOrder(orderId: String): OrderDTO {
        val order = orderService.getOrder(OrderCommand.GetById(orderId))
        val user = userService.getUser(UserCommand.GetById(order.userId))
        val product = productService.getProduct(ProductCommand.GetById(order.productId))
        
        // 비즈니스 로직 처리
        // ...
        
        return OrderDTO(order, user, product)
    }
}
```
