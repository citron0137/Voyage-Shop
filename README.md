...
# 주문 상품 순위 테스트 데이터 생성 도구

이 프로젝트는 주문 상품 순위 API 테스트를 위한 더미 데이터 생성 도구를 포함하고 있습니다.

## 주요 기능

- 데이터베이스에 대량의 주문 데이터 직접 주입
- 주문 아이템 데이터 생성 및 인기 상품 패턴 설정
- 주문 상품 순위 API 테스트를 위한 환경 구성
...
- 애플리케이션이 최소 한 번 실행되어 테이블이 생성되어 있어야 합니다.
- bash 쉘이 지원되는 환경(Linux, macOS, WSL, Git Bash 등)이 필요합니다.

### 데이터 생성 실행

```bash
# 스크립트 실행 권한 설정
chmod +x script/generate-order-items-rank-data.sh

# 기본 설정으로 실행 (localhost:5432, DB명: voyageshop, 계정: postgres/postgres)
./script/generate-order-items-rank-data.sh

# 사용자 정의 DB 설정으로 실행
./script/generate-order-items-rank-data.sh [DB_HOST] [DB_PORT] [DB_NAME] [DB_USER] [DB_PASSWORD]
```

이 스크립트는 다음 작업을 수행합니다:

1. 약 10,000개의 주문 데이터 생성
2. 약 30,000개의 주문 아이템 데이터 생성
3. 약 3,000개의 주문 할인 데이터 생성
4. 인기 상품 데이터 패턴 생성
5. 최근 주문 데이터에 인기 상품 추가
6. 순위 API 데이터 초기화 호출

### 테스트 실행

데이터 생성 후, 순위 테스트를 다음과 같이 실행할 수 있습니다:

```bash
# 기본 순위 테스트 실행
k6 run k6-tests/order-item-rank-test.js

# 캐시 효율성 테스트 실행
k6 run k6-tests/order-item-rank-cache-test.js

# 스파이크 테스트 실행
k6 run k6-tests/order-item-rank-spike-test.js
```

## 파일 구조

- `data/order-data-generator.sql`: 주문 데이터 생성 SQL 스크립트
- `script/generate-order-items-rank-data.sh`: 데이터 생성 실행 스크립트
- `k6-tests/`: k6 테스트 스크립트 폴더

## 트러블슈팅

- **테이블 오류**: 애플리케이션을 먼저 실행하여 필요한 테이블(orders, order_items, order_discounts)이 생성되어 있는지 확인하세요.
- **DB 연결 실패**: 데이터베이스 연결 정보가 올바른지 확인하세요.
- **SQL 스크립트 오류**: `/tmp/order_data_generation.log` 파일에서 자세한 오류 정보를 확인할 수 있습니다.
- **API 초기화 실패**: 애플리케이션 서버가 실행 중인지 확인하세요.
